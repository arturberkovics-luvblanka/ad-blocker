...
- Add Safari's `popup` resource type to `$popup` rules while retaining the
  `document` fallback needed for link-created tabs. Preserve explicit content
  types, including a separate child-frame trigger for `$subdocument`.
