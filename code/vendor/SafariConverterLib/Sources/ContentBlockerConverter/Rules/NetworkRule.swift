...
public class NetworkRule: Rule {
    ...
    private func loadOptions(options: String, version: SafariVersion) throws {
        ...
        // A $popup rule without an explicit content type also targets documents internally.
        if !enabledOptions.isDisjoint(with: .documentLevel) {
            let popupHasExplicitContentType =
                isOptionEnabled(option: .popup)
                && permittedContentType != .all
            if permittedContentType != .subdocument && !popupHasExplicitContentType {
