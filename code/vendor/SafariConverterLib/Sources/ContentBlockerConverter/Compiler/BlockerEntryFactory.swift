...
class BlockerEntryFactory {
    ...
    private func prepareNetworkRulesTriggers(
        ...
        let popupHasSubdocument =
            rule.isOptionEnabled(option: .popup)
            && rule.hasContentType(contentType: .subdocument)
        let popupHasDocument =
            rule.isOptionEnabled(option: .popup)
            && rule.isOptionEnabled(option: .document)
        if !popupHasSubdocument && !popupHasDocument {
        ...
        var baseTriggers = [baseTrigger]
        if popupHasSubdocument && !popupHasDocument {
            var subdocumentTrigger = baseTrigger
            subdocumentTrigger.resourceType = ["document"]
            addLoadContext(rule: rule, trigger: &subdocumentTrigger)
            baseTriggers.append(subdocumentTrigger)
        }

        return try baseTriggers.flatMap {
            try createTriggersWithDomainOptions(rule: rule, baseTrigger: $0)
        }
    }
    ...
    private func addResourceType(rule: NetworkRule, trigger: inout BlockerEntry.Trigger) throws {
        ...
        if rule.isOptionEnabled(option: .popup) {
            let usesSeparateSubdocumentTrigger =
                rule.hasContentType(contentType: .subdocument)
                && !rule.isOptionEnabled(option: .document)
            if !usesSeparateSubdocumentTrigger && !types.contains("document") {
                types.append("document")
            }
            types.append("popup")
        }
