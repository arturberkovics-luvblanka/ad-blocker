...
- `$popup` rules use Safari's `popup` and `document` resource types. The popup
  type covers script-created windows, while the document fallback is needed for
  link-created tabs and can also block matching document loads in frames.
  Explicit content types remain active; a combined `$subdocument,popup` rule
  becomes separate popup and child-frame triggers.
