...
final class BlockerEntryFactoryTests: XCTestCase {
    ...
    func testDocumentLevelRules() {
        let testCases: [TestCase] = [
            ...
            TestCase(
                // Explicit $document remains effective alongside $popup.
                ruleText: "||example.org^$document,popup",
                expectedEntry: BlockerEntry(
                    trigger: BlockerEntry.Trigger(
                        urlFilter: #"^[^:]+://+([^:/]+\.)?example\.org[/:]"#,
                        resourceType: ["document", "popup"]
            ...
            TestCase(
                // Explicit content types remain effective alongside $popup.
                ruleText: "||example.org^$script,popup",
                expectedEntry: BlockerEntry(
                    trigger: BlockerEntry.Trigger(
                        urlFilter: #"^[^:]+://+([^:/]+\.)?example\.org[/:]"#,
                        resourceType: ["script", "document", "popup"]
                    ),
                    action: BlockerEntry.Action(
                        type: "block"
                    )
            ...
            TestCase(
                // $subdocument needs a separate child-frame trigger from $popup.
                ruleText: "||example.org^$subdocument,popup",
                version: SafariVersion.safari15,
                expectedEntries: [
                    BlockerEntry(
                        trigger: BlockerEntry.Trigger(
