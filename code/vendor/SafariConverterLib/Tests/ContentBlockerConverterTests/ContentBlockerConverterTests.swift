...
final class ContentBlockerConverterTests: XCTestCase {
    ...
    func testPinnedPopupModifierCombinations() throws {
        let rules = [
            "/l/DMP_picture_captcha?$document,popup",
            "&modver=*&appver=$document,popup",
            "&appver=*&modver=$document,popup",
            "/captha/cap/|$document,popup",
            "/in/p/?spot_id=$document,popup",
            "?dovr=true|$document,popup",
            "||freefuckvidz.com/d/$script,popup",
            "||css-load.com^$document,popup,badfilter",
        ...
        let result = ContentBlockerConverter().convertArray(
            rules: rules,
            safariVersion: .safari26
        )
        let entries = try parseJsonString(json: result.safariRulesJSON)

        XCTAssertEqual(result.sourceRulesCount, 16)
        XCTAssertEqual(result.sourceSafariCompatibleRulesCount, 16)
        XCTAssertEqual(result.safariRulesCount, 15)
        XCTAssertEqual(result.errorsCount, 0)
        XCTAssertEqual(
            entries.filter { $0.trigger.resourceType == ["document", "popup"] }.count,
            12
        )
        XCTAssertEqual(
            entries.filter {
                $0.trigger.resourceType == ["script", "document", "popup"]
            }.count,
            1
        )
        XCTAssertEqual(
            entries.filter { $0.trigger.resourceType == ["popup"] }.count,
            1
        )
        XCTAssertEqual(
            entries.filter {
                $0.trigger.resourceType == ["document"]
                    && $0.trigger.loadContext == ["child-frame"]
            }.count,
            1
        )
    }
